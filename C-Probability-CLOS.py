...
Samples     = 1000      # number of time units
mode        = 3         # 0: Random, 1: LLA, 2: TTA, 3: NEW
debug       = 0
debug1      = 0
debug2      = 0
f = open("data.txt", "w")
results = 'mode=' + str(mode) + '\n' + 'N\tInLoad\tORate\tUtil\tAvgDly\tRelDly\n'

setup = []
for N in range(8, 9, 2):
    M = 2**N
    #for Load in (1*M//8, 2*M//8, 3*M//8, 4*M//8):#, 5*M//8, 6*M//8, 7*M//8):

    for Load in (7 * M // 8, 8):

        setup.append([2**N, Load])
...
for s in setup:
    N = s[0]                # network size
    Load = s[1]          # input load 0-100
    print(N, Load, Samples)
    ...
    for i in range (Samples):
        ...
        for ind in range (N):
            ...
            # find path
            path_exists = ADD_CLOS_path (sqrt_N, port_num, port_dem)
            if path_exists==1:
                success_cnt += 1
